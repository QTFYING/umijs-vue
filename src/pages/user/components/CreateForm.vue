<template>
    <a-modal :visible="visible" :maskClosable="false" :confirmLoading="confirmLoading" :destroyOnClose="true"
        :width="800" :title="opt === 'add' ? t('user.button.add') : t('user.button.edit')" @ok="onOk"
        @cancel="onCancel">
        <a-form ref="modalFormRef" name="myform" :model="userInfo" v-bind="layout">
            ...
            <a-form-item name="username" :label="t('user.column.username')"
                :rules="[{ required: true, message: t('user.form.message.username') }]">
                <a-input v-model:value="userInfo.username" :maxlength="50"
                    :placeholder="t('user.form.message.username')" autocomplete="off"
                    :disabled="opt === 'add' ? false : true" />
            </a-form-item>
            <a-form-item name="password" :label="t('user.form.password')" :rules="[
                {
                ...
                <a-input-password v-model:value="userInfo.password" :maxlength="50" autocomplete="off" :placeholder="opt === 'update'
                    ? t('user.form.message.update')
                    : t('user.form.message.password')
                    " />
            </a-form-item>
            <a-form-item name="confirmPassword" :label="t('user.form.confirmPassword')" :rules="[
                {
                    required: opt === 'update' ? false : true,
                    trigger: 'change',
                ...
                <a-input-password v-model:value="userInfo.confirmPassword" :maxlength="50" autocomplete="off"
                    :placeholder="opt === 'update'
                        ? t('user.form.message.update')
                        : t('user.form.message.confirmPassword')
                        " />
            </a-form-item>
            <a-form-item name="realName" :label="t('user.column.realName')"
                :rules="[{ required: true, message: t('user.form.message.realName') }]">
                <a-input v-model:value="userInfo.realName" :maxlength="50"
                    :placeholder="t('user.form.message.realName')" />
            </a-form-item>
            <a-form-item name="sex" :label="t('user.column.sex')"
                :rules="[{ required: true, message: t('user.form.message.sex') }]">
                <a-select v-model:value="userInfo.sex" :placeholder="t('user.form.message.sex')"
                    :options="user.sexList ?? []">
                </a-select>
            </a-form-item>
            <a-form-item name="role" :label="t('user.column.role')"
                :rules="[{ required: true, message: t('user.form.message.role') }]">
                <a-select v-model:value="userInfo.role" :placeholder="t('user.form.message.role')"
                    :options="user.roleList ?? []">
                </a-select>
            </a-form-item>
            <a-form-item name="gameList" :label="t('user.column.gameList')"
                :rules="[{ required: true, message: t('user.form.message.gameList') }]">
                <a-select v-model:value="userInfo.gameList" mode="multiple"
            ...
            <a-form-item name="telephone" :label="t('user.column.telephone')">
                <a-input v-model:value="userInfo.telephone" :maxlength="20"
                    :placeholder="t('user.form.message.telephone')" />
            </a-form-item>
            <a-form-item name="email" :label="t('user.column.email')"
                :rules="[{ trigger: 'change', validator: validateEmailInput }]">
                <a-input v-model:value="userInfo.email" :maxlength="50" :placeholder="t('user.form.message.email')" />
            </a-form-item>
            <a-form-item name="remark" :label="t('user.column.remark')">
                <a-textarea v-model:value="userInfo.remark" show-count :maxlength="200"
